...
async function loadSidebarUser() {
  try {
    ...
    // Admin Panel link isn't in every page's static sidebar markup (only
    // admin.html has it) — inject it for admins so it's reachable from
    // anywhere, without showing it to students.
    if (user.role === 'admin' && !document.querySelector('.sidebar-link[href="admin.html"]')) {
      ...
      if (nav && userCard) {
        ...
        link.innerHTML = '<i class="bi bi-shield-lock"></i> Admin Panel';
