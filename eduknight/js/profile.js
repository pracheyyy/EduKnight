...
async function loadProfile() {
  try {
    const res = await fetch(`${API_BASE_URL}/users/me/profile`, { credentials: 'include', cache: 'no-store' });
...
function renderProfile(data) {
  ...
  if (user.avatarUrl) {
    avatarEl.innerHTML = `<img src="${user.avatarUrl}" alt="${user.name}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;" onerror="this.parentElement.textContent='${initials(user.name)}';">`;
  } else {
...
function initEditForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // Re-fetch the full profile instead of manually patching a few DOM
      // nodes — guarantees every part of the page (stats, badges, sidebar,
      // form) reflects exactly what's now in the database, not just the
      // two fields this form happens to touch.
      await loadProfile();
  ...
  // "Change photo" button — previously had no handler at all, so clicking
  // it silently did nothing. There's no file-upload/storage backend yet
  // (that needs multer + S3/Cloudinary, a bigger addition), so this asks
  // for an image URL as an interim way to actually set avatarUrl.
  document.getElementById('avatarEditBtn')?.addEventListener('click', async () => {
    const { value: url, isConfirmed } = await Swal.fire({
      title: 'Set profile photo',
      input: 'text',
      inputLabel: 'Image URL',
      inputPlaceholder: 'https://example.com/your-photo.jpg',
      showCancelButton: true,
      confirmButtonText: 'Save',
      confirmButtonColor: '#2563EB',
      inputValidator: (value) => {
        if (value && !/^https?:\/\/.+/i.test(value)) return 'Enter a valid image URL, or leave blank to remove your photo.';
      },
    });
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json()).message || 'Could not update photo.');
